...
relative_vendor_dir = Path(sys.argv[3])
...
if expected_variant != 'cpu':
    link_patterns.append('libonnxruntime_providers_cuda.so*')

for pattern in link_patterns:
    sources = list(vendor_dir.glob(pattern))
    assert sources
    for source in sources:
        (capi_dir / source.name).symlink_to(relative_vendor_dir / source.name)
