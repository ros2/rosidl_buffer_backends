...
def test_vendored_runtime():
    ...
    expected_variant = os.environ['PYTHON_ONNXRUNTIME_VENDOR_VARIANT']
    vendor_dir = Path(
        os.environ['PYTHON_ONNXRUNTIME_VENDOR_LIBDIR']).resolve()
    ...
    assert all(path.resolve().is_relative_to(vendor_dir) for path in native_libraries)
    ...
    assert all(path.is_relative_to(vendor_dir) for path in mapped_libraries)
