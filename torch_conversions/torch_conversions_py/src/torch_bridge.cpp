...
{
  ManagedContext(
    std::vector<int64_t> shape_arg,
    std::vector<int64_t> strides_arg,
    py::object owner_arg)
  : shape(std::move(shape_arg)),
    strides(std::move(strides_arg)),
...
py::capsule make_dlpack_capsule(
  ...
  py::object owner)
{
  auto context = std::make_unique<ManagedContext>(
    std::move(shape), std::move(strides), std::move(owner));
...
PYBIND11_MODULE(_torch_bridge, module)
{
  module.def(
