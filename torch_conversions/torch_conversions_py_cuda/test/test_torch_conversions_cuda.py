...
def test_bridge_build_has_no_cuda_or_native_buffer_dependency():
    core_root = Path(__file__).parents[2] / 'torch_conversions_py'
    source = (core_root / 'src/dlpack_bridge.cpp').read_text()
    cmake = (core_root / 'CMakeLists.txt').read_text()
    manifest = (core_root / 'package.xml').read_text()
