...
production_packages=(
  torch-conversions
  torch-conversions-cpu
  torch-conversions-cuda
  torch-conversions-py
  torch-conversions-py-cpu
  torch-conversions-py-cuda
)
...
docker run --rm --gpus all \
  ...
  --mount "type=bind,src=${script_dir}/run-offline-test.sh,dst=/usr/local/bin/run-offline-test.sh,readonly" \
  --mount "type=bind,src=${script_dir}/test-installed-debians.sh,dst=/usr/local/bin/test-installed-debians.sh,readonly" \
  --entrypoint bash \
  "${test_image}" \
  /usr/local/bin/run-offline-test.sh
