...
TEST(OnnxRuntimeConversions, RunsCudaInferenceWithMessageBuffers)
{
  ...
  {
    auto input_writer = from_output_tensor_msg(input, memory_info, stream);
    ASSERT_EQ(
      cudaMemcpyAsync(
        input_writer.value().GetTensorMutableRawData(), expected.data(),
  ...
  std::vector<float> actual(expected.size());
  {
    ...
    auto output_reader = from_input_tensor_msg(
      const_output, memory_info, stream);
    ASSERT_EQ(
      cudaMemcpyAsync(
        actual.data(), output_reader.value().GetTensorRawData(),
        actual.size() * sizeof(float), cudaMemcpyDeviceToHost, stream),
      cudaSuccess);
  }
  ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
